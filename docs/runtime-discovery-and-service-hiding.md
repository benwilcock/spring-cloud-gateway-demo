...
Download and install all the "things you'll need" from the list above. Then `cd` to the `runtime-discovery` folder in the source code as follows:
...
Build & package the gateway, registry, and service into JARs using Maven, and then create Docker containers for each of them. We have provided a handy `pack-images.sh` script to do this:
...
Start up the Docker test environment in the background. We’re using `docker-compose` as it can start multiple containers and create a private network for them to communicate on.
...
Wait. Docker will start all the containers (using the configuration provided in the `docker-compose.yml` file). Waiting a couple of extra minutes here is advised. It gives Docker time to start everything and gives the applications a chance to communicate and settle down.
...
In this demo, we have 3 servers. These servers are all running inside a "hidden" network which is provided by Docker Compose. Only the [Gateway server][5] is exposed to the outside world, so all HTTP traffic must go via this Gateway.
...
As you can see in the [`docker-compose.yml` configuration][8], Docker is configured to only allow external calls to reach the Gateway – on port `80`. The other servers (the Registry, and the Greeting Service), cannot be reached directly from outside.
